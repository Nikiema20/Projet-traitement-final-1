    ...
    def traiter_table(self, table): # table sera une instance de la classe table
        ...
        if self.date_debut==self.date_fin:
            ...
            for ligne in table.lignes:
                if ligne[indice_date]==date_debut: #un problème pourrait se poser lors du test, comment entrons nous date_debut et fin pour la comparaison
                    L.append(ligne)
        else:
            for ligne in table.lignes:
                if ligne[indice_date] > date_debut and ligne[indice_date] < date_fin:
                    L.append(ligne)

        return Table(nom_colonnes,L) #ici je veux retourner un objet de type table 
